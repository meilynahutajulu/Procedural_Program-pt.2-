  ...
  while(y == 0){
    ...
    if (strcmp(tok, "assign-student") == 0)
     ...
     if (strcmp(tok, "move-student")==0) {
            ...
            for (int i = 0; i < std; i++){
                if(strcmp(dorm[i].name, dn)==0){
                    idx_d = i;
                    
                    break;
                }
            }
            if (student[idx_s].dorm==NULL){
                assign_student(&student[idx_s], &dorm[idx_d], id, dn);
            } else {
                for (int i = 0; i < dx; i++){
                    if(strcmp(student[idx_s].dorm->name, dorm[i].name)==0){
                        move_student(&student[idx_s], &dorm[idx_d], &dorm[i], id, dn);
                        break;
                    }
                }
            }
